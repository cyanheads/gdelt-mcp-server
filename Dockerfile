...
# Install only production dependencies, ignoring lifecycle scripts and optional
# framework peer tiers that this server does not import.
RUN --mount=type=cache,target=/root/.bun/install/cache \
...
RUN mkdir -p /var/log/gdelt-mcp-server && chown -R bun:bun /var/log/gdelt-mcp-server
...
ENV LOGS_DIR="/var/log/gdelt-mcp-server"
